...
void SymbolVisitor::visit(AST *ast) {
  ...
  for (auto doc : documents) {
    if (doc) {
      switch (doc->type) {
        case DocumentDecl::Type::Struct:
          structs.push_back(doc);
          break;
        case DocumentDecl::Type::Defination:
          definations.push_back(doc);
          break;
        case DocumentDecl::Type::Instance:
          instances.push_back(doc);
          break;
      }
    }
  }
  ...
  ComponentInstanceDecl *cid =
      dynamic_cast<ComponentInstanceDecl *>(instances[0]);
  assert(cid != nullptr);
  m_topLevelInstance = cid;

  Scope *mainScope =
      new Scope(Scope::Category::Function, m_ast->symbolTable()->curScope());
...
void SymbolVisitor::visit(ComponentInstanceDecl *cid) {
  ...
  Scope *instanceScope =
      new Scope(Scope::Category::Instance, m_ast->symbolTable()->curScope());
  instanceScope->setScopeName(cid->instanceId);
  Symbol *componentSymbol =
      m_ast->symbolTable()->curScope()->resolve(cid->componentName);
  if (!componentSymbol) {
  ...
  ComponentDefinationDecl *cdd =
      dynamic_cast<ComponentDefinationDecl *>(componentDef);
  assert(cdd != nullptr);
  ...
  if (cid->parent) {
    Symbol *parentSymbol = new Symbol(
        Symbol::Category::InstanceId, "parent",
...
void SymbolVisitor::calculateOrderedMemberInitList() {
  ...
  for (auto &p : m_bindingId2bindingDecl) {
    ...
    util::condPrint(option::printBindingDep, "binding: filepath [%d] %s\n", seq,
                    m_topLevelInstance->filepath.c_str());
    util::condPrint(option::printBindingDep, "binding: seq [%d] %s(%p)\n", seq,
                    id.c_str(), astNode);
  }

  vector<ComponentInstanceDecl *> instances =
      m_topLevelInstance->instanceList();
  for (auto instance : instances) {
    ...
    for (int propertyIndex : unbound) {
      ...
      ASTNode *astNode =
          cdd->propertyList[static_cast<size_t>(propertyIndex)].get();
  ...
  for (auto &p : m_bindingIdDeps) {
    ...
    util::condPrint(option::printBindingDep,
                    "binding: edge %d -> %d(%s -> %s)\n", fromSeq, toSeq,
                    fromId.c_str(), toId.c_str());
  }
  ...
  if (result == TopologicalSorter::SortResult::LoopDetected) {
    ...
    assert(ret);
    throw SyntaxError("Loop detected in property dependency",
                      seq2astNode[node]->token(), seq2filepath[node]);
  }
  ...
  for (int i = 0; i < static_cast<int>(order.size()); i++) {
    ...
    m_topLevelInstance->orderedMemberInitList.push_back(
        make_pair(cid, astNode));
    util::condPrint(option::printBindingDep,
                    "binding: order [%d] [%d] %s(%p)\n", i, seq, id.c_str(),
                    astNode);
  }
}
...
void SymbolVisitor::visit(BindingDecl *bd) {
  ...
  if (!propertySymbol) {
    string msg = "Component \"" + componentSymbol->name() +
  ...
  m_bindingId2bindingDecl[bindingId(curInstanceId(), bindingIndexAnalyzing())] =
      bd;
  visit(bd->expr.get());
...
void SymbolVisitor::visitInstanceId(ComponentInstanceDecl *cid) {
  ...
  Symbol *symbol = new Symbol(
      Symbol::Category::InstanceId, id,
      shared_ptr<TypeInfo>(new CustomTypeInfo(cid->componentName)), cid);
  ...
  switch (b->op) {
    ...
    case BinaryOperatorExpr::Op::LogicalOr: {
      if (leftType->category() != TypeInfo::Category::Int) {
        throw SyntaxError("'&&' / '||' operator require int operand",
                          b->left->token(), m_curFilePath);
      }
      if (rightType->category() != TypeInfo::Category::Int) {
        throw SyntaxError("'&&' / '||' operator require int operand",
                          b->right->token(), m_curFilePath);
      }
    ...
    case BinaryOperatorExpr::Op::GreaterEqual: {
      if (*leftType != *rightType) {
        const string msg =
            "'<' / '>' / '<=' / '>=' operator require type of operands is the "
            "same";
      ...
      if (cat == TypeInfo::Category::Int || cat == TypeInfo::Category::Float) {
        ...
        const string msg =
            "'<' / '>' / '<=' / '>=' operator require type of operands is int "
            "/ float";
    ...
    case BinaryOperatorExpr::Op::NotEqual: {
      if (*leftType != *rightType) {
        const string msg =
            "'==' / '!=' operator require type of operands is the same";
        throw SyntaxError(msg, b->right->token(), m_curFilePath);
      }
      TypeInfo::Category cat = leftType->category();
      if (cat == TypeInfo::Category::Int || cat == TypeInfo::Category::Float ||
          cat == TypeInfo::Category::String) {
        // good
      } else {
        const string msg =
            "'==' / '!=' operator require type of operands is int / float / "
            "string";
        throw SyntaxError(msg, b->left->token(), m_curFilePath);
      }
      b->typeInfo = shared_ptr<TypeInfo>(new TypeInfo(TypeInfo::Category::Int));
    ...
    case BinaryOperatorExpr::Op::Plus: {
      ...
      if (cat == TypeInfo::Category::Int || cat == TypeInfo::Category::Float ||
        ...
        const string msg =
            "'+' operator require type of operands is int / float / string";
        throw SyntaxError(msg, b->left->token(), m_curFilePath);
      }
      b->typeInfo = leftType;
      break;
  ...
  switch (u->op) {
    ...
    case UnaryOperatorExpr::Op::Positive: {
      if (eType->category() == TypeInfo::Category::Int ||
        ...
        const string msg =
            "Unary operator '-' / '+' require type of operand is int / float";
...
void SymbolVisitor::visit(CallExpr *e) {
  ...
  if (e->funcExpr->category == Expr::Category::Ref) {
    ...
    Symbol *instanceTypeSymbol =
        m_ast->symbolTable()->curScope()->resolve(typeName);
    ...
    if (!method) {
      const string msg =
          "Type \"" + typeName + "\" has no method named \"" + m->name + "\"";
  ...
  if (paramTypes.size() != e->paramList.size()) {
    ...
    snprintf(buf, sizeof(buf),
             "Function \"%s\" requires %d parameters but is passed %d",
             functionName.c_str(), static_cast<int>(paramTypes.size()),
  ...
  for (size_t i = 0; i < paramTypes.size(); i++) {
    ...
    if (*paramTypes[i] != *(e->paramList[i]->typeInfo)) {
      char buf[512];
      snprintf(buf, sizeof(buf),
               "Function %s requires type \"%s\" in %dth parameter but is "
               "passed \"%s\"",
               functionName.c_str(), paramTypes[i]->toString().c_str(),
               static_cast<int>(i),
...
void SymbolVisitor::visit(ListSubscriptExpr *e) {
  ...
  if (e->indexExpr->typeInfo->category() != TypeInfo::Category::Int) {
    throw SyntaxError("Type of list index should be int", e->indexExpr->token(),
                      m_curFilePath);
  }
...
void SymbolVisitor::visit(MemberExpr *e) {
  ...
  if (instanceTypeInfo->category() == TypeInfo::Category::Custom) {
    ...
    const string msg =
        "Type \"" + instanceTypeInfo->toString() + "\" has no member";
    throw SyntaxError(msg, e->instanceExpr->token(), m_curFilePath);
  }
  ...
  if (!memberSymbol) {
    const string msg =
        "Type \"" + typeString + "\" has no member named \"" + e->name + "\"";
    throw SyntaxError(msg, e->token(), m_curFilePath);
  }
  ...
  if (memberSymbol->category() == Symbol::Category::Property &&
      analyzingPropertyDep()) {
    ASTNode *ast = memberSymbol->astNode();
    PropertyDecl *pd = dynamic_cast<PropertyDecl *>(ast);
    assert(pd != nullptr);

    componentDefinationAnalyzing()
        ->propertyDeps[propertyIndexAnalyzing()]
        .insert(pd->fieldIndex);
  ...
  if (analyzingBindingDep()) {
    if (memberSymbol->category() == Symbol::Category::Property) {
      ...
      util::condPrint(
          option::printBindingDep, "binding: %s[%d](%p) -> %s[%d]\n",
          curInstanceId().c_str(), bindingIndexAnalyzing(), bindingAnalyzing(),
          m_curAnalyzingBindingToId.c_str(), pd->fieldIndex);
      m_bindingIdDeps.push_back(pair<string, string>(
          bindingId(curInstanceId(), bindingIndexAnalyzing()),
  ...
  if (analyzingBindingDep()) {
    if (sym->category() == Symbol::Category::Property) {
      ...
      util::condPrint(
          option::printBindingDep, "binding: %s[%d](%p) -> %s[%d]\n",
          curInstanceId().c_str(), bindingIndexAnalyzing(), bindingAnalyzing(),
          curInstanceId().c_str(), pd->fieldIndex);
      m_bindingIdDeps.push_back(pair<string, string>(
          bindingId(curInstanceId(), bindingIndexAnalyzing()),
...
void SymbolVisitor::visit(VarDecl *vd) {
  ...
  Symbol *variableSym =
      new Symbol(Symbol::Category::Variable, vd->name, vd->type, vd);
  m_ast->symbolTable()->define(variableSym);

  if (vd->type->category() == TypeInfo::Category::Custom) {
    Symbol *typeSymbol =
        m_ast->symbolTable()->curScope()->resolve(vd->type->toString());
    if (!typeSymbol) {
      // FIXME
    ...
    switch (typeSymbol->category()) {
      ...
      case Symbol::Category::Enum:
        throw SyntaxError("Please use int type instead of enum type",
                          vd->token(), m_curFilePath);
      case Symbol::Category::Component:
        throw SyntaxError("Define variable with component type is illegal",
                          vd->token(), m_curFilePath);
      default:
        throw SyntaxError(
            "\"" + typeSymbol->name() + "\" is a " +
                Symbol::symbolCategoryString(typeSymbol->category()) +
                ", not a type",
            vd->token(), m_curFilePath);
    }
  }
  ...
  util::condPrint(option::printGenAsm, "genAsm: localIndex [%d] %s\n",
                  vd->localIndex, vd->name.c_str());

  if (vd->expr) {
    visit(vd->expr.get());
    if (!(*vd->type == *vd->expr->typeInfo) &&
        !vd->type->assignCompatible(vd->expr->typeInfo)) {
...
void SymbolVisitor::visitPropertyDefination(PropertyDecl *pd) {
  ...
  Symbol *propertySym =
      new Symbol(Symbol::Category::Property, propertyName, pd->type, pd);
...
void SymbolVisitor::visit(ParamDecl *pd) {
  ...
  Symbol *paramSym =
      new Symbol(Symbol::Category::Parameter, pd->name, pd->type, pd);
...
void SymbolVisitor::visit(CompoundStmt *cs) {
  ...
  Scope *localScope(
      new Scope(Scope::Category::Local, m_ast->symbolTable()->curScope()));
...
void SymbolVisitor::visit(IfStmt *is) {
  ...
  if (is->condition->typeInfo->category() != TypeInfo::Category::Int) {
    throw SyntaxError(
        "If statment requires type of condition expression is int",
        is->condition->token(), m_curFilePath);
  }
...
void SymbolVisitor::visit(BreakStmt *bs) { assert(bs != nullptr); }

void SymbolVisitor::visit(ContinueStmt *cs) { assert(cs != nullptr); }
...
void SymbolVisitor::visitMethodHeader(FunctionDecl *fd) {
  ...
  Symbol *methodSym(
      new MethodSymbol(fd->name, fd->returnType, componentSymbol, paramTypes));
...
void SymbolVisitor::visitMethodBody(FunctionDecl *fd) {
  ...
  Scope *methodScope(
      new Scope(Scope::Category::Method, m_ast->symbolTable()->curScope()));
...
void SymbolVisitor::visit(EnumConstantDecl *ecd) {
  ...
  Symbol *enumConstantSym =
      new Symbol(Symbol::Category::EnumConstants, ecd->name,
                 make_shared<TypeInfo>(TypeInfo::Category::Int), ecd);
...
void SymbolVisitor::visit(EnumDecl *ed) {
  ...
  Symbol *enumSym = new Symbol(Symbol::Category::Enum, ed->name,
                               std::shared_ptr<TypeInfo>(), ed);
...
void SymbolVisitor::visit(FunctionDecl *) { assert(false); }

void SymbolVisitor::visit(PropertyDecl *) { assert(false); }
