...
struct InitListExpr : public Expr {
  explicit InitListExpr(std::vector<std::unique_ptr<Expr>> &&l)
      : Expr(Category::InitList), exprList(move(l)) {}
...
struct BinaryOperatorExpr : public Expr {
  ...
  frontend::Token token() const override { return left->token(); }
...
struct UnaryOperatorExpr : public Expr {
  enum Op { Invalid, Positive, Negative, Not };

  UnaryOperatorExpr() : Expr(Category::UnaryOperator) {}
  static std::string typeString(Op op);

  frontend::Token token() const override { return expr->token(); }
...
struct ParamDecl : public ASTNode {
  ParamDecl(const std::string &n, const std::shared_ptr<backend::TypeInfo> &t)
      : name(n), type(t) {}
...
struct CompoundStmt : public Stmt {
  explicit CompoundStmt(std::vector<std::unique_ptr<Stmt>> &&sl)
      : Stmt(Category::Compound), stmtList(move(sl)) {}

  std::vector<std::unique_ptr<Stmt>> stmtList;
};

struct DeclStmt : public Stmt {
  explicit DeclStmt(std::unique_ptr<VarDecl> &&vd)
      : Stmt(Category::Decl), decl(std::move(vd)) {}

  std::unique_ptr<VarDecl> decl;
};

struct IfStmt : public Stmt {
  IfStmt(std::unique_ptr<Expr> &&cond, std::unique_ptr<Stmt> &&ts,
...
struct ReturnStmt : public Stmt {
  explicit ReturnStmt(std::unique_ptr<Expr> &&re)
      : Stmt(Category::Return), returnExpr(move(re)) {}
...
struct ExprStmt : public Stmt {
  explicit ExprStmt(std::unique_ptr<Expr> &&e)
      : Stmt(Category::Expr), expr(move(e)) {}
...
struct DocumentDecl : public ASTNode {
  enum class Type { Defination, Instance, Struct };
...
struct FieldDecl : public ASTNode {
  FieldDecl(const std::string &name_,
            const std::shared_ptr<backend::TypeInfo> &type_)
...
struct ComponentInstanceDecl : public DocumentDecl {
  ...
  std::vector<std::pair<ComponentInstanceDecl *, ASTNode *>>
      orderedMemberInitList;
};
