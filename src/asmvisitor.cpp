...
AsmVisitor::AsmVisitor() { m_visitingLvalueStack.push_back(false); }
...
void AsmVisitor::visit(BinaryOperatorExpr *boe) {
  ...
  if (boe->op == BinaryOperatorExpr::Op::Assign) {
    ...
    switch (boe->left->typeInfo->category()) {
      case TypeInfo::Category::Int:
        prefix = "i";
        break;
      case TypeInfo::Category::Float:
        prefix = "f";
        break;
...
void AsmVisitor::visit(CallExpr *ce) {
  ...
  if (functionName == "len" || functionName == "print" ||
      functionName == "defineScene" || functionName == "drawRect" ||
      functionName == "drawText" || functionName == "drawEllipse" ||
      functionName == "drawPolygon" || functionName == "drawLine" ||
  ...
  if (visitingLvalue()) {
    ...
    switch (symbol->category()) {
      ...
      case Symbol::Category::InstanceId: {
        ComponentInstanceDecl *cid =
            dynamic_cast<ComponentInstanceDecl *>(astNode);
        assert(cid != nullptr);
...
void AsmVisitor::visit(ParamDecl *pd) { assert(pd != nullptr); }
...
void AsmVisitor::visit(EnumConstantDecl *ecd) { assert(ecd != nullptr); }
...
void AsmVisitor::visit(FieldDecl *fd) { assert(fd != nullptr); }

void AsmVisitor::visit(StructDecl *sd) { assert(sd != nullptr); }
...
void AsmVisitor::genAsmForInitInstance(ComponentInstanceDecl *cid) {
  ...
  for (auto instance : instances) {
    ...
    int fieldCount =
        static_cast<int>(instance->componentDefination->propertyList.size());
...
void AsmVisitor::genAsmForPropertyDecl(ComponentInstanceDecl *cid,
                                       PropertyDecl *pd) {
  int instanceIndex = cid->instanceIndex;
...
void AsmVisitor::genAsmForBindingDecl(ComponentInstanceDecl *cid,
                                      BindingDecl *bd) {
  int instanceIndex = cid->instanceIndex;
  assert(instanceIndex != -1);
...
bool AsmVisitor::visitingLvalue() const { return m_visitingLvalueStack.back(); }

void AsmVisitor::setVisitingInstance(bool visiting,
                                     ComponentDefinationDecl *cdd, int index) {
...
bool AsmVisitor::visitingInstance() const { return m_visitingInstance; }

int AsmVisitor::instanceIndexVisiting() const {
  return m_instanceIndexVisiting;
}

bool AsmVisitor::visitingMethod() const { return m_visitingMethod; }
