...
Token::TokenType Parser::tokenType(int i) const { return token(i).type; }
...
void Parser::incTrying() { m_trying++; }

void Parser::decTrying() { m_trying--; }
...
static const set<int> memberItemFirst = {
    Token::T_INT,  Token::T_VOID, Token::T_FLOAT,     Token::T_STRING,
    Token::T_LIST, Token::T_ENUM, Token::T_IDENTIFIER};
...
static const set<int> propertyTypeFirst = {Token::T_INT, Token::T_FLOAT,
                                           Token::T_STRING, Token::T_LIST};
static const set<int> propertyDefinationFirst = propertyTypeFirst;

static const set<int> typeFirst = {Token::T_INT,    Token::T_FLOAT,
                                   Token::T_STRING, Token::T_LIST,
                                   Token::T_VOID,   Token::T_IDENTIFIER};
...
void Parser::parseMemberItem(unique_ptr<ComponentDefinationDecl> &defination) {
  ...
  if (curToken().is(Token::T_ENUM)) {
    ...
    const char *msg =
        "Expect a enumDefination / propertyDefination / functionDefination";
    throw SyntaxError(msg, curToken());
  }
...
bool Parser::tryMemberItemAlt1() {
  ...
  try {
    ...
    util::condPrint(option::printLLTry, "tryMemberItemAlt1 fail: %s\n",
                    e.what());
    result = false;
  }
...
bool Parser::tryMemberItemAlt2() {
  ...
  try {
    ...
    util::condPrint(option::printLLTry, "tryMemberItemAlt2 fail: %s\n",
                    e.what());
    result = false;
    throw;
  }
...
std::unique_ptr<FunctionDecl> Parser::parseFunctionDefination() {
  ...
  if (!trying()) {
    ...
    decl.reset(
        new FunctionDecl(name, move(ti), move(paramList),
                         unique_ptr<CompoundStmt>(
                             dynamic_cast<CompoundStmt *>(body.release()))));
    decl->tok = tok;
  }
  return decl;
}
...
static const set<int> blockItemFirst = {Token::T_INT,
                                        Token::T_VOID,
                                        Token::T_FLOAT,
                                        Token::T_STRING,
                                        Token::T_LIST,
...
//    | statement     // LBrace | If | While | Continue | Break | Return |
//    Identifier | StringLiteral | NumberLiteral | LParen
//    ;
...
bool Parser::tryBlockItemAlt1() {
  ...
  try {
    ...
    util::condPrint(option::printLLTry, "tryBlockItemAlt1 fail: %s\n",
                    e.what());
    result = false;
    throw;
  }
...
bool Parser::tryBlockItemAlt2() {
  ...
  try {
    ...
    util::condPrint(option::printLLTry, "tryBlockItemAlt2 fail: %s\n",
                    e.what());
    result = false;
  }
...
static const set<int> expressionFirst = {
    Token::T_IDENTIFIER, Token::T_STRING_LITERAL, Token::T_NUMBER_LITERAL,
    Token::T_L_PAREN,    Token::T_PLUS,           Token::T_MINUS,
    Token::T_NOT};

static const set<int> initializerListFirst = {
    Token::T_IDENTIFIER, Token::T_STRING_LITERAL, Token::T_NUMBER_LITERAL,
    Token::T_L_PAREN,    Token::T_PLUS,           Token::T_MINUS,
    Token::T_NOT,        Token::T_L_BRACE};
...
// expression  // Identifier | StringLiteral | NumberLiteral | LParen | Plus |
// Minus | Not
//    : logicalOrExpression                   // Identifier | StringLiteral |
//    NumberLiteral | LParen | Plus | Minus | Not | postfixExpression Assign
//    expression   // Identifier | StringLiteral | NumberLiteral | LParen | Plus
//    | Minus | Not
//    ;
...
std::unique_ptr<Expr> Parser::parseRelationalExpression() {
  ...
  while (
      curToken().isIn({Token::T_LT, Token::T_GT, Token::T_LE, Token::T_GE})) {
    match(curTokenType());
    tokens.push_back(token(m_index - 1).type);
    subExprs.push_back(parseAdditiveExpression());
  }
...
static const set<int> unaryOperatorFirst = {Token::T_PLUS, Token::T_MINUS,
                                            Token::T_NOT};
...
std::unique_ptr<Expr> Parser::parsePostfixExpression() {
  enum PostfixType { Call, Subscript, Member };
  ...
  while (
      curToken().isIn({Token::T_L_BRACKET, Token::T_L_PAREN, Token::T_DOT})) {
  ...
  if (!trying()) {
    ...
    if (subExprCount == 1) {
      ...
      for (size_t i = 0; i < subExprCount - 1; i++) {
        ...
        if (types[i] == Call) {
          dynamic_cast<CallExpr *>(p)->funcExpr = move(subExprs[i]);
        } else if (types[i] == Subscript) {
          dynamic_cast<ListSubscriptExpr *>(p)->listExpr = move(subExprs[i]);
        } else if (types[i] == Member) {
          dynamic_cast<MemberExpr *>(p)->instanceExpr = move(subExprs[i]);
        } else {
          assert(false);
        }
      }
...
void Parser::parseBindingItemList(
    std::unique_ptr<ComponentInstanceDecl> &instanceDecl) {
