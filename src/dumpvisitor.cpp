...
void DumpVisitor::visit(FunctionDecl *fd) {
  IndentPrinter ip(this);
  if (fd->component) {
    printf("FunctionDecl(%s %s::%s)\n", fd->returnType->toString().c_str(),
           fd->component->name.c_str(), fd->name.c_str());
  } else {
...
void DumpVisitor::incIndent() { m_indent++; }

void DumpVisitor::decIndent() { m_indent--; }
