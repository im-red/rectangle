...
AsmBin::AsmBin(const AsmText &t) { assemble(t); }
...
void AsmBin::assemble(const AsmText &t) {
  ...
  for (auto &line : text) {
    ...
    if (firstWord == ".def") {
      ...
      instr::AsmInstruction ins =
          static_cast<instr::AsmInstruction>(instr::getAsmValue(firstWord));
...
void AsmBin::dump() {
  ...
  for (size_t i = 0; i < m_constants.size(); i++) {
    printf("    %04x: %s\n", static_cast<unsigned>(i),
           m_constants[i].toString().c_str());
  }

  printf("Functions:\n");
  for (size_t i = 0; i < m_functions.size(); i++) {
    FunctionItem &f = m_functions[i];
    printf("    %04x: %08x %s %d %d\n", static_cast<unsigned>(i), f.addr,
           f.name.c_str(), f.args, f.locals);
  }
  ...
  while (offset < static_cast<int>(m_code.size())) {
    ...
    if (instr::is0OpInstr(instr)) {
      ...
      printf("    %04x: %02x             ; %-9s\n", addr, instr,
             instr::getAsmName(instr).c_str());
      offset += 1;
    } else if (instr::is1OpInstr(instr)) {
      unsigned addr = static_cast<unsigned>(offset);
      int op = getInt(offset + 1);
      printf("    %04x: %02x %02x %02x %02x %02x ; %-9s %x\n", offset, instr,
             m_code[addr + 1], m_code[addr + 2], m_code[addr + 3],
             m_code[addr + 4], instr::getAsmName(instr).c_str(), op);
      offset += 5;
    } else {
      printf("%d\n", instr);
      assert(false);
...
int AsmBin::codeSize() const { return static_cast<int>(m_code.size()); }
...
int AsmBin::defineFunction(const std::string &name, int addr, int args,
                           int locals) {
  size_t index = 0;
  ...
  if (isRef) {
    ...
    util::condPrint(option::printAssemble,
                    "assemble: def [%u] function %s %d %d %d\n", index,
                    printName.c_str(), addr, args, locals);
  }
